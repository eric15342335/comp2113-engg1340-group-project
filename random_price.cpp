...
float percentage_change_price(Stock & stock) {
    ...
    float mean = stock.get_attribute(mean) + stock.sum_attribute(mean);
    float standard_deviation = stock.get_attribute(standard_deviation) + stock.sum_attribute(standard_deviation);
    ...
    float z_score_upper_limit = (upper_limit - mean) / standard_deviation;
    float z_score_lower_limit = (lower_limit - mean) / standard_deviation;
    for (int i = 0; i < 5; i++) {
